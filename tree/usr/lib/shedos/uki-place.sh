...
# (only when the box has a db cert — a non-Secure-Boot box ships its unsigned
# UKI as-is), then rename(2) over the target so the prior boot image survives
# until the new one is proven good. Space-aware: refuse rather than tear a UKI
# onto a full FAT volume. The caller exports DB_CERT (the sbverify cert, empty
# or absent on a keyless box) and SHEDOS_ESP_DIRS.
...
# _place SRC DST — atomic, signature-verified (when DB_CERT exists), space-aware.
...
_place() {
    ...
    if [[ -f ${DB_CERT:-} ]] && ! sbverify --cert "$DB_CERT" "$tmp" >/dev/null 2>&1; then
